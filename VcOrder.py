...
keys = [
    ...
    ['Delete', 'Place Order', 'Print Receipt', 'Voice Order']
]
...
menu_items = {
    ...
    "veg burger": ["veg burger", "vegetable burger","Vegitable","Veg"],
    "fries": ["fries", "french fries"],
    "coke": ["coke", "coca cola", "cola", "cok"]
}
...
def draw_keys(img, key_highlight=None):
    ...
    for i, key in enumerate(keys[0]):
        ...
        cv2.putText(overlay, key, (x + w // 10, y + h // 2 + 10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
...
def map_speech_to_items(speech_text):
    speech_text = speech_text.lower()
    added_items = []
    for item, aliases in menu_items.items():
        for alias in aliases:
            if alias in speech_text:
...
def process_voice_command():
    ...
    with sr.Microphone() as source:
        engine.say("Please say your order")
        engine.runAndWait()
        audio = recognizer.listen(source)
    try:
        speech_text = recognizer.recognize_google(audio)
        print("You said:", speech_text)
        items = map_speech_to_items(speech_text)
        if items:
            for item in items:
                typed_text += item + " "
                ordered_items.append(item)
            engine.say(f"Added {', '.join(items)} to your order.")
        else:
            engine.say("No recognizable items found in your order.")
        engine.runAndWait()
    except sr.UnknownValueError:
        engine.say("Sorry, I could not understand your order.")
        engine.runAndWait()
            ...
            if lm_list:
                ...
                if distance < PINCH_THRESHOLD:
                    ...
                    if key_pressed and (current_time - last_press_time > press_delay):
                        ...
                        elif key_pressed == "Voice Order":
                            process_voice_command()
                        else:
                            item = key_pressed.lower()
                            typed_text += item + " "
                            ordered_items.append(item)
